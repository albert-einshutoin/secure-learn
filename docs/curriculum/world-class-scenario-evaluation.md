...
Secure Learn は、基礎シナリオ S1-S15 と高度シナリオ S16-S33 により、ホワイトハット、SRE、バックエンド、検知、Cloud/IaC、Supply Chain、Release Governance を同じ閉域ラボで順番に学べる構成になっています。

重要な前提として、このリポジトリは安全なローカル/コンテナ教材です。本物の業務熟達には、許可された実cloud account、production-like telemetry、組織のreview/incident processで追加訓練が必要です。ただし、HTMLに沿って全フェーズを実施すれば、実務面接や配属初期で説明すべき判断軸と証跡の作り方は一通り通せます。
...
| S16-S21 | Linux internals、eBPF/perf、TCP/LB、TLS/mTLS、Edge、Kubernetes platform | OS/network/platformを本番運用のguardrailとして扱う |
...
| S28-S33 | API business logic、supply chain、detection/EDR、performance、GitOps/OSS governance | 世界レベルのsecure SDLCとrelease/開示運用へ接続する |

## HTMLハンズオン

各シナリオの実行フロー、抽象説明、具体例、初学者の見方、経験者の深掘り、学習フロー図、事前準備、安全境界、観測ポイント、よくある失敗、セルフレビュー、ツール活用、合格証跡は [Scenario HTML Guides](../scenario-guides/index.html) で確認できます。

各フェーズの順番、Docker profile、初学者の見方、経験者の深掘り、学習フロー図、事前準備、安全境界、観測ポイント、よくある失敗、セルフレビュー、合格証跡は [Learning Phase Guides](../learning-phases/index.html) で確認できます。
...
`scripts/world_class_hands_on_check.sh all` は、Linux internals、network edge、Kubernetes、cloud、IaC、observability、distributed systems、backend、API security、supply chain、detection、endpoint、performance、OSS governance の各領域について、ローカルで確認できる入口と証跡を生成します。
