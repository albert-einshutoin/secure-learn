...
| Linux | ローカルまたはrootless Docker Engine 28.1.0以上（API 1.49以上） | `/var/run/docker.sock` または実行ユーザーの `/run/user/<uid>/docker.sock` |

全OSでDocker Engine 28.1.0／API 1.49と正式版Docker Compose 2.36.0以上が必要です。Docker Desktop 4.42.0はDocker Engine 28.2.2とDocker Compose 2.36.2を同梱する導入目安であり、doctorはDesktop製品バージョンではなくEngine/APIとruntime capabilityを直接判定します。`interface_name` はCompose側の構文対応に加えてEngine 28.1のendpoint interface name APIを必要とするため、doctorはバージョン番号、Compose設定の構文展開、一時コンテナ内で `eth0` / `eth1` が存在し `eth2` が存在しないことを実測します。一時projectは呼出ごとのランダム名で分離され、成功・失敗を問わず同じ完全一致名だけがcleanupされます。
