...
doctorはclient-sideの `compose config` だけを成功根拠にしません。呼出ごとに暗号学的ランダムsuffixを持つ一時projectを起動し、隔離されたAlpineコンテナ内でapp/data networkがそれぞれ `eth0` / `eth1` と指定IPへ実装されたことを観測します。cleanupは同じ呼出で生成した完全一致projectだけを対象にし、成功・失敗のどちらでもコンテナ、network、volumeを削除します。このruntime probeを含むため、初回は固定digestのAlpine image取得時間がかかることがあります。
