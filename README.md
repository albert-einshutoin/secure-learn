...
- macOS/Windows: Docker Desktop 4.42.0以上を導入目安とし、実判定は下記のEngine/API・Compose・runtime capability gateを使用
- Linux: Docker Engine 28.1.0以上（API 1.49以上）とDocker Compose 2.36.0以上
- 8GB以上のメモリ
- 20GB以上のディスク空き容量

Docker Engine 28.1.0／API 1.49とDocker Compose 2.36.0はdoctorが数値検査する最低契約です。IDSが監視する攻撃網を常に `eth0` に固定するため、Docker Compose 2.36.0で導入された [`interface_name`](https://docs.docker.com/reference/compose-file/services/#interface_name) と、Engine 28.1で導入されたendpoint interface name APIを使用します。[Docker公式リリースノート](https://docs.docker.com/desktop/release-notes/#4420) 上、Docker Desktop 4.42.0はDocker Engine 28.2.2とDocker Compose 2.36.2を同梱するため導入目安にしていますが、doctorはDesktop製品バージョン自体を取得せず、Engine/API、Compose正式版SemVer、runtime capabilityを直接判定します。
...
doctorはclient-sideの `compose config` だけを成功根拠にしません。呼出ごとに暗号学的ランダムsuffixを持つ一時projectを起動し、隔離されたAlpineコンテナ内でapp/data networkがそれぞれ `eth0` / `eth1` として実装されたことを観測します。probe専用networkのIPAMはDockerへ委譲し、並行project同士の固定subnet衝突を避けます。cleanupは同じ呼出で生成した完全一致projectだけを対象にし、成功・失敗のどちらでもコンテナ、network、volumeを削除します。このruntime probeを含むため、初回は固定digestのAlpine image取得時間がかかることがあります。

POSIXホストではsocketを `lstat` し、symlinkではないsocket種別とcontextに対応する所有者（rootfulはroot、rootless/macOSは実行者）を確認します。socketのgroupは実行者のprimaryまたはsupplementary groupでなければならず、other-writeは常に拒否します。接続権限は、実行者がownerならowner-write、ownerでなければsocket groupへの所属とgroup-writeの両方を必須とします。そのため、所属groupに限定された `0660` は許可しますが、foreign groupや `0777` は拒否します。ただし、ローカルsocketの先にあるpeerが別ホストへのproxyではないことを暗号学的に証明する仕組みではありません。doctorはローカル設定のfail-closed検査であり、ホスト管理者や同一権限ユーザーが改変したdaemon／socketの真正性をattestするものではありません。
