...
- Linux: Docker Engine 20.10.0以上（API 1.41以上）とDocker Compose 2.36.0以上
- 8GB以上のメモリ
- 20GB以上のディスク空き容量

Engine 20.10.0／API 1.41とCompose 2.36.0はdoctorが数値検査する最低契約です。IDSが監視する攻撃網を常に `eth0` に固定するため、Docker Compose 2.36.0で導入された [`interface_name`](https://docs.docker.com/reference/compose-file/services/#interface_name) を使用します。[Docker公式リリースノート](https://docs.docker.com/desktop/release-notes/#4420) 上、Docker Desktop 4.42.0はDocker Compose 2.36.2を同梱するため導入目安にしていますが、doctorはDesktop製品バージョン自体を取得せず、Engine/API、Compose正式版SemVer、runtime capabilityを直接判定します。
